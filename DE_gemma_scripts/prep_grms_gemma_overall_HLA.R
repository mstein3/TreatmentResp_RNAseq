...
clean_grm <- function(grm_raw, females_pheno, males_pheno, write_loc, filter_write_loc){
  ...
  write.table(final_grm, file = paste(write_loc, "HLA.overall.gemma.grm", sep = ""), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
  ...
  write.table(combined_id, file = paste(filter_write_loc, "HLA.overall.iids.txt", sep = ""), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
