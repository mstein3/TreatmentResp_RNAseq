...
for (gender in genders) {
  for (grm_type in grm_types) {
    ...
    ids_filter <- read.table(paste(filter_dir, "HLA.", gender, ".", grm_type, ".iids.txt", sep = ""), sep = "\t", header = FALSE, stringsAsFactors = FALSE)
    ...
    #Read in expr files
    auto_expr_raw <- read.table(paste(inp_dir, "HLA_TMMvoom_NullLPS_GX_224_20190223.txt", sep = ""), sep = "\t", header = TRUE)
    #filter expr files
    ...
    expr_raw <- auto_expr_raw[,paste("X", pheno_raw$Extract_Ord, sep = "")]
    ...
    for (i in 1:nrow(expr_raw)) {
      ...
      #write files
      write.table(pheno, file = paste("/scratch/mconery/sex-specific/expression_data/", "HLA.", gender, ".", rownames(expr_raw)[i], ".", grm_type, ".txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
      write.table(pheno_treatment, file = paste("/scratch/mconery/sex-specific/expression_data/", "HLA.", gender, ".", rownames(expr_raw)[i], ".", grm_type, ".treatment.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
      write.table(pheno_no_treatment, file = paste("/scratch/mconery/sex-specific/expression_data/", "HLA.", gender, ".", rownames(expr_raw)[i], ".", grm_type, ".no.treatment.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
