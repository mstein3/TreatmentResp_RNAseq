...
# Purpose: Run GEMMA for differential expression by treatmentas a main effect using BIMBAM file format. 

# Usage: Usage: Gemma_bimbam_IntAsMainEffect.R <counts> <covar_file> <relat> <geno_file> <gene_name> <dir_pheno> <dir_gemma> 
...
#
# geno_file: mean genotype file in BIMBAM file format. In this case, the genotype is a treatment by sex interaction . 
#
...
nl_int_subdir <- paste0("TreatbySex_coded01", "/")
...
#f_top <- file.path(dir_gemma, paste0("top-bb-nl-treat.txt"))
...
run_gemma <- function(genotype_file, pheno_file, relatedness, covar, out_prefix, outdir) {
  cmd <- sprintf("gemma -g %s -p %s -k %s -km 1 -notsnp -c %s -lmm 4 -o %s",
                 genotype_file, pheno_file, relatedness, covar, out_prefix)
  system(cmd)
...
message("Running GEMMA")
  f_gemma <- run_gemma(genotype_file = f_genotype, pheno_file = f_pheno_g, relatedness = f_relat, covar = f_covar,
                       out_prefix = paste0("TreatbySex_bb-", g),
